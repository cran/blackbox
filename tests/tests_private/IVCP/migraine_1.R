...
## Migraine version 0.5 (Built on Sep  2 2016 at 11:16:56).
...
GP$graphicsExt <- paste(".", tolower("pdf"), sep="")
...
GP$FONKgNames <- c("twoNmu","twoNm","g")
GP$FONKgScale <- c("Canonical","Canonical","Canonical")
names(GP$FONKgScale) <- GP$FONKgNames
GP$samplingSpace<-c("twoNmu","twoNm","g") ## ONLY for next points
GP$samplingScale<-c("Canonical","Canonical","Canonical") ## ONLY for next points
...
GP$graphicPars<-list()
GP$plotOptions<-c("")
GP$oneDimCIvars<-c("twoNmu","twoNm","g","Nb") ## (an empty c() value will be meaningful for nextBounts)
GP$spec1DProfiles<-c()
GP$spec2DProfiles<-c()
GP$S2factor<-1 ##  other values add more confusion that anything else
GP$D2IBDbool<-FALSE;GP$D1IBDbool<-TRUE 
GP$Nbfactor<-692.006 ##  correction from 2NmS2 to 2Ds2=Nb
GP$GeoUnit<-"ind.m" ## string that gives the Nb unit in plot
GP$mincondS2<-1
...
write("Migraine 0.5 (Built on Sep  2 2016 at 11:16:56)",file=GP$cleanResu)
...
write("Data file: IVCP",file=GP$cleanResu)
